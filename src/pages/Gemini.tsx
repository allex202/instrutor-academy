...
import { geminiLessons } from '../data/modules/gemini';
import ProgressBar from '../components/ui/ProgressBar';
import Badge from '../components/ui/Badge';
import { CheckCircle2, Circle, ChevronDown, ChevronUp, BookOpen, MessageSquare, Check, Lightbulb, Hexagon } from 'lucide-react';

export default function Gemini() {
  const { progress, dispatch } = useProgress();
  const [expandedLesson, setExpandedLesson] = useState<string | null>(null);
  
  const moduleId = 'gemini';
  
  const completedLessons = progress.completedLessons.filter(l => l.startsWith(moduleId + '/'));
  const moduleProgress = Math.round((completedLessons.length / (geminiLessons?.length || 1)) * 100) || 0;
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-12 animate-slide-up">
      <header className="bg-white dark:bg-neutral-800 p-8 rounded-2xl border border-stone-200 dark:border-neutral-700 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start gap-6">
          <div>
            <Badge variant="default" className="bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400 mb-4">
              Módulo 3.6
            </Badge>
            <h1 className="text-3xl md:text-4xl font-bold text-stone-900 dark:text-stone-100 mb-4 flex items-center gap-3">
              <Hexagon className="w-10 h-10 text-cyan-600" />
              Ecossistema Google (Gemini)
            </h1>
            <p className="text-stone-500 dark:text-stone-400 max-w-2xl text-lg">
              Explore a arquitetura Multimodal, Ring Attention e os Agentes Autônomos (Spark) da Google DeepMind.
            </p>
          </div>
          <div className="w-full md:w-64 bg-stone-50 dark:bg-neutral-900 p-4 rounded-xl border border-stone-200 dark:border-neutral-700">
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-xl font-bold text-cyan-600 dark:text-cyan-500">{moduleProgress}%</span>
            </div>
            <ProgressBar value={moduleProgress} size="sm" color="blue" showPercentage={false} />
            <p className="text-xs text-stone-500 mt-2 text-right">
              {completedLessons.length} de {geminiLessons?.length || 0} lições
            </p>
          </div>
        </div>
      </header>
      ...
      <section>
        ...
        <div className="space-y-4">
          {geminiLessons?.map((lesson, index) => {
                ...
                >
                  <div className="flex items-center gap-4">
                    <button 
                      ...
                      className={`shrink-0 rounded-full transition-colors ${isCompleted ? 'text-cyan-500' : 'text-stone-300 dark:text-neutral-600 hover:text-cyan-400'}`}
                    >
                    ...
                    <div className="prose dark:prose-invert max-w-none space-y-6 mt-6">
                      {lesson.content?.sections?.map(section => (
                        <div key={section.id}>
                          ...
                          {section.type === 'code' && (
                             <pre className="bg-stone-900 text-cyan-400 p-4 rounded-xl overflow-x-auto border border-stone-800 font-mono text-sm">
                               {section.content}
                             </pre>
                          )}
                          ...
                          {section.type === 'callout' && (
                            <div className={`p-4 rounded-xl border my-4 ${
                              section.calloutType === 'tip' ? 'bg-cyan-50 border-cyan-200 text-cyan-900 dark:bg-cyan-900/20 dark:border-cyan-800/50 dark:text-cyan-100' :
                              'bg-amber-50 border-amber-200 text-amber-900 dark:bg-amber-900/20 dark:border-amber-800/50 dark:text-amber-100'
                            }`}>
                              <h4 className="font-bold mb-2 flex items-center gap-2">
                                {section.calloutType === 'tip' ? <Lightbulb className="w-5 h-5" /> : <MessageSquare className="w-5 h-5" />}
                                {section.title}
                              </h4>
                              {/* Layout bug safely avoided by not hardcoding heights */}
