...
import { apiLessons, codeExamples } from '../data/modules/api-development';
import { ModuleQuiz } from '../components/shared/ModuleQuiz';
import ProgressBar from '../components/ui/ProgressBar';
import Badge from '../components/ui/Badge';
import CodeBlock from '../components/ui/CodeBlock';
import TabGroup from '../components/ui/TabGroup';
import APISandbox from '../components/shared/APISandbox';
import { CheckCircle2, Circle, ChevronDown, ChevronUp, BookOpen, Code, Check } from 'lucide-react';
...
export default function APIDevelopment() {
  ...
  const moduleId = 'api-development';
  
  const completedLessons = progress.completedLessons.filter(l => l.startsWith(moduleId + '/'));
  const moduleProgress = Math.round((completedLessons.length / (apiLessons?.length || 1)) * 100) || 0;
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-12 animate-slide-up">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start gap-6">
          <div>
            ...
            <p className="text-stone-500 dark:text-stone-400 max-w-2xl text-lg">
              Integre o Claude em suas próprias aplicações utilizando a Messages API da Anthropic.
            </p>
          </div>
          <div className="w-full md:w-64 bg-stone-50 dark:bg-neutral-900 p-4 rounded-xl border border-stone-200 dark:border-neutral-700">
            ...
            <p className="text-xs text-stone-500 mt-2 text-right">
              {completedLessons.length} de {apiLessons?.length || 0} lições
            </p>
          </div>
        </div>
      </header>
      ...
      <section>
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-2xl font-bold text-stone-900 dark:text-stone-100">Exemplos de Código</h2>
        </div>
        
        <div className="bg-white dark:bg-neutral-800 rounded-xl border border-stone-200 dark:border-neutral-700 overflow-hidden shadow-sm">
          ...
          <div className="p-6">
            {codeExamples?.map((example, i) => (
              <div key={i} className="mb-6 last:mb-0">
                <h3 className="font-bold text-stone-900 dark:text-stone-100 mb-2">{example.title}</h3>
                <p className="text-stone-600 dark:text-stone-400 mb-4">{example.description}</p>
                <CodeBlock code={example[activeTab as "python" | "javascript" | "curl"] || ""} language={activeTab as any} />
              </div>
            ))}
            {!codeExamples && (
              <p className="text-stone-500">Nenhum exemplo disponível ainda.</p>
            )}
          </div>
        </div>
      </section>
