...
import { openaiLessons } from '../data/modules/openai';
import { ModuleQuiz } from '../components/shared/ModuleQuiz';
import ProgressBar from '../components/ui/ProgressBar';
import Badge from '../components/ui/Badge';
import CodeBlock from '../components/ui/CodeBlock';
import { CheckCircle2, Circle, ChevronDown, ChevronUp, BookOpen, MessageSquare, Check, Lightbulb } from 'lucide-react';
...
export default function OpenAI() {
  ...
  const moduleProgress = Math.round((completedLessons.length / (openaiLessons?.length || 1)) * 100) || 0;
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-12 animate-slide-up">
      <header className="bg-white dark:bg-neutral-800 p-8 rounded-2xl border border-stone-200 dark:border-neutral-700 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start gap-6">
          <div>
            <Badge variant="default" className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 mb-4">
              Módulo 3.5
            </Badge>
            ...
            <p className="text-stone-500 dark:text-stone-400 max-w-2xl text-lg">
              Aprenda a arquitetura, modelos e engenharia de prompt do maior concorrente de mercado para expandir sua visão como Instrutor.
            </p>
          </div>
          <div className="w-full md:w-64 bg-stone-50 dark:bg-neutral-900 p-4 rounded-xl border border-stone-200 dark:border-neutral-700">
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-xl font-bold text-emerald-600 dark:text-emerald-500">{moduleProgress}%</span>
            </div>
            <ProgressBar value={moduleProgress} size="sm" color="emerald" showPercentage={false} />
            <p className="text-xs text-stone-500 mt-2 text-right">
              {completedLessons.length} de {openaiLessons?.length || 0} lições
            </p>
          </div>
        </div>
      </header>
      ...
      <section>
        ...
        <div className="space-y-4">
          {openaiLessons?.map((lesson, index) => {
      ...
      {/* Como ainda não injetamos quiz específico para OpenAI, removemos o bloco de ModuleQuiz para evitar bugs, 
          ou podemos manter se quisermos reaproveitar depois. Removido por segurança. */}
    </div>
  );
}
